...
protected[core] abstract class EntitlementProvider(config: WhiskConfig, loadBalancer: LoadBalancer, iam: NamespaceProvider)(
    ...
    protected[core] def checkThrottles(user: Identity)(
        ...
        checkSystemOverload(ACTIVATE) orElse {
            hasTooManyRequests(!invokeRateThrottler.check(subject))
        } orElse {
            hasTooManyConcurrentRequests(!concurrentInvokeThrottler.check(subject))
        } map {
            Future.failed(_)
     ...
     */
    protected def checkSystemOverload(right: Privilege)(implicit transid: TransactionId) = {
    ...
    private def checkUserThrottle(subject: Subject, right: Privilege, resources: Set[Resource])(
        ...
        hasTooManyRequests(right == ACTIVATE && userThrottled)
    }

    /** Helper. */
    private def hasTooManyRequests(yes: Boolean)(implicit transid: TransactionId) = {
        if (yes) {
            Some(RejectRequest(TooManyRequests, tooManyRequests))
        } else None
    }
    ...
    private def checkConcurrentUserThrottle(subject: Subject, right: Privilege, resources: Set[Resource])(
        ...
        hasTooManyConcurrentRequests(right == ACTIVATE && userThrottled)
    }

    /** Helper. */
    private def hasTooManyConcurrentRequests(yes: Boolean)(implicit transid: TransactionId) = {
        if (yes) {
