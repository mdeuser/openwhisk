...
install "$CATALOG_HOME/samples/hello.js" \
     samples/helloWorld \
     -a 'description','Demonstrates logging facilities' \
     -a 'parameters','"[{'name': 'payload', 'required':false, 'description':'"The string to be included in the log record"'}]"' \
...
install "$CATALOG_HOME/samples/wc.js" \
     samples/wordCount \
     -a 'description','Count words in a string' \
     -a 'parameters','"[{'name': 'payload', 'required':true, 'description':'"A string"'}]"' \
     -a 'sampleInput','"{ 'payload': '"Five fuzzy felines"'}"' \
     -a 'sampleOutput','"{ 'councleart': 3 }"' \
...
install "$CATALOG_HOME/samples/echo.js" \
    samples/echo \
    -a 'description','Returns the input' \
